...
class Saml2RelyingPartyRegistrationConfiguration {
	...
	private RelyingPartyRegistration asRegistration(String id, Registration properties) {
		...
		builder.assertingPartyDetails(
				(details) -> details.verificationX509Credentials((credentials) ->
					 properties.getAssertingparty()
					.getVerification()
					.getCredentials()
					.stream()
		...
		validateSigningCredentials(properties, true);
		return registration;
	}
	...
	private Consumer<AssertingPartyDetails.Builder> mapAssertingParty(AssertingParty assertingParty,
		...
		return (details) -> {
			...
			map.from(assertingParty.getSinglesignon()::isSignRequest)
				...
				.to(wantAuthnRequestsSigned -> details.wantAuthnRequestsSigned(true));
